...
- **Legacy slash commands removed** — `/brainstorm`, `/execute-plan`, and `/write-plan` are gone. They were deprecated stubs that did nothing but tell the user to invoke the corresponding skill. Invoke `superpowers-deepseek-v4:brainstorming`, `superpowers-deepseek-v4:executing-plans`, and `superpowers-deepseek-v4:writing-plans` directly instead. (#1188)
- **`superpowers-deepseek-v4:code-reviewer` named agent removed** — the agent was the plugin's only named agent and was used by exactly two skills, while every other reviewer/implementer subagent in the repo dispatches `general-purpose` with a prompt template alongside its skill. The agent's persona and checklist have been merged into `skills/requesting-code-review/code-reviewer.md` as a self-contained Task-dispatch template. Anyone dispatching `Task (superpowers-deepseek-v4:code-reviewer)` should switch to `Task (general-purpose)` with the prompt template instead. (PR #1299)
...
Added explicit priority ordering to using-superpowers-deepseek-v4:
...
The underlying skills (`superpowers-deepseek-v4:brainstorming`, `superpowers-deepseek-v4:executing-plans`, `superpowers-deepseek-v4:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
- Updated slash commands to use fully qualified skill names (e.g., `superpowers-deepseek-v4:brainstorming`)
...
- Namespaced skills: `superpowers-deepseek-v4:skill-name` for superpowers skills, `skill-name` for personal
...
- Added `superpowers-deepseek-v4:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `superpowers-deepseek-v4:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `superpowers-deepseek-v4:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `superpowers-deepseek-v4:code-reviewer`
...
- All internal skill references now use `superpowers-deepseek-v4:` namespace prefix
- Updated format: `superpowers-deepseek-v4:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/superpowers-deepseek-v4:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
